    ...
    return (
      <section className="bg-white py-24 px-6 lg:px-12">
        <div className="max-w-7xl mx-auto flex flex-col items-center justify-center text-center gap-4 py-16">
          ...
          <p className="text-[14px] text-neutral-400 font-normal max-w-sm leading-[1.8]">
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-12">
          ...
          {events.length > 1 && (
            <div className="flex items-center gap-3">
              ...
              <button
                onClick={prev}
                className="w-10 h-10 border border-neutral-200 hover:border-neutral-900 flex items-center justify-center transition-colors duration-200 group"
              >
                <ChevronLeft size={16} className="text-neutral-400 group-hover:text-neutral-900 transition-colors" />
              </button>
              <button
                onClick={next}
                className="w-10 h-10 border border-neutral-200 hover:border-neutral-900 flex items-center justify-center transition-colors duration-200 group"
        ...
        <AnimatePresence mode="wait" custom={direction}>
          <motion.div
            ...
            className="grid grid-cols-1 lg:grid-cols-2 gap-0 min-h-[480px]"
          >
            {/* Image side */}
            <div className="relative overflow-hidden bg-neutral-100 min-h-[300px] lg:min-h-full">
              {event.image ? (
                <Image
              ...
              <div className="absolute top-5 left-5">
                <span className="bg-red-600 text-white text-[10px] font-medium tracking-[1.5px] uppercase px-3 py-1.5">
            ...
            {/* Content side */}
            <div className="bg-neutral-950 p-10 lg:p-14 flex flex-col justify-center gap-6">

              {/* Date */}
              <div className="flex flex-col gap-1">
                <span className="text-[10px] font-medium tracking-[3px] uppercase text-red-500">
                  {format(new Date(event.date), "EEEE")}
                </span>
                <span className="text-[32px] font-extrabold text-white leading-none tracking-tight">
                  {format(new Date(event.date), "MMMM d, yyyy")}
                </span>
                <span className="text-[14px] text-white/50 font-normal mt-1">
              ...
              {/* Title */}
              <h2 className="text-[clamp(22px,3vw,36px)] font-extrabold text-white leading-tight uppercase tracking-tight">
                {event.title}
              </h2>

              {/* Description */}
              {event.description && (
                <p className="text-[14px] text-white/60 font-normal leading-[1.85] max-w-md">
                  {event.description}
                </p>
              )}

              {/* Location */}
              <div className="flex items-start gap-3 mt-auto pt-4 border-t border-white/08">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#E30000" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="mt-0.5 shrink-0">
                  <path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0118 0z" />
                  <circle cx="12" cy="10" r="3" />
                </svg>
                <span className="text-[13px] text-white/60 font-normal leading-[1.7]">
                  {event.location}
                </span>
              </div>
              ...
              <button
                ...
                className={`transition-all duration-300 ${
                  i === current
                    ...
                    : "w-1.5 h-1.5 rounded-full bg-neutral-200 hover:bg-neutral-400"
                }`}
              />
            ))}
          </div>
